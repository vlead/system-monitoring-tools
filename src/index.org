...
* What to monitoring?
 - Checking cluster server status details on [[http://nagios.vlabs.ac.in/nagios/][nagios]]. (for example
   memory status and other services like SSH, HTTP, ping, current
   users..etc)
...
* How to monitoring?
 - Monitoring cluster server details like total Hosts/Services,
